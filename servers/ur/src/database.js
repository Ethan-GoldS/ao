...
export async function getServerInfo() {
  ...
  try {
    const result = await pool.query('SELECT start_time, total_requests FROM ur_metrics_server_info WHERE id = 1')
    if (result.rows.length > 0) {
      return {
...
/**
 * Get process counts from database
 * @returns {Promise<Object>} Process counts
 */
export async function getProcessCounts() {
  if (!pool) return {}
  ...
  try {
    const result = await pool.query('SELECT process_id, count FROM ur_metrics_process_counts')
    const counts = {}
    result.rows.forEach(row => {
      counts[row.process_id] = parseInt(row.count, 10) || 0
    })
    return counts
  } catch (err) {
    _logger('Error getting process counts: %O', err)
...
export async function getActionCounts() {
  ...
  try {
    const result = await pool.query('SELECT action, count FROM ur_metrics_action_counts')
    const counts = {}
    result.rows.forEach(row => {
      counts[row.action || 'unknown'] = parseInt(row.count, 10) || 0
    })
    return counts
  } catch (err) {
    _logger('Error getting action counts: %O', err)
...
export async function getIpCounts() {
  ...
  try {
    const result = await pool.query('SELECT ip, count FROM ur_metrics_ip_counts ORDER BY count DESC LIMIT 100')
    const counts = {}
    result.rows.forEach(row => {
      counts[row.ip || 'unknown'] = parseInt(row.count, 10) || 0
    })
    return counts
  } catch (err) {
    _logger('Error getting IP counts: %O', err)
...
/**
 * Get referrer counts from database
 * @returns {Promise<Object>} Referrer counts
 */
export async function getReferrerCounts() {
  if (!pool) return {}
  ...
  try {
    const result = await pool.query('SELECT referrer, count FROM ur_metrics_referrer_counts ORDER BY count DESC LIMIT 100')
    const counts = {}
    result.rows.forEach(row => {
      counts[row.referrer || 'unknown'] = parseInt(row.count, 10) || 0
    })
    return counts
  } catch (err) {
    _logger('Error getting referrer counts: %O', err)
...
export async function getTimeSeriesData() {
  ...
  try {
    // Get the most recent time bucket entries, sorted by timestamp
    const result = await pool.query('SELECT timestamp, hour, total_requests, process_counts FROM ur_metrics_time_series ORDER BY timestamp DESC LIMIT 24')
...
export async function getRecentRequests() {
  ...
  try {
    const result = await pool.query('SELECT timestamp, process_id, ip, action, duration FROM ur_metrics_requests ORDER BY timestamp DESC LIMIT 100')
